 ...
 **数据库表（15 张）：**
 ...
 knowledge_embeddings        向量嵌入（已弃用，保留兼容）
 ...
 **工业级分块器** 代码在 `app/web_ingestion/industrial_chunker.rs`（20,610 字节），
 ...
 使用配置的 Embedding 模型（如 `nomic-embed-text`），把文本块变成向量。
 向量存储到 `knowledge_chunks` 表，向量数据存到 `knowledge_embeddings` 表。
 ...
 **代码位置**：`src/infra/web_ingestion/fetcher.rs`（23,045 字节）
 ...
 在 `config.toml` 的 `[web_ingestion]` 段：
 ...
 auto_publish = true
 auto_publish_min_score = 0.65   # 自动发布的最低质量分
 ...
 # 调度周期
 scheduler_interval_secs = 900    # 调度器每 15 分钟检查一次
 dispatcher_interval_secs = 5     # 分发器每 5 秒轮询一次
 ...
 ### 11.3 为什么要有 Qdrant 同步？DB 不是也有数据吗？
 ...
 **knowledge_chunks** —— 文档被切成的文本块
 ...
 token_count INT,
 metadata JSON,
 embedding VECTOR,               -- 向量数据（MySQL 8.4+）
 status TINYINT,                 -- 0=暂存, 1=已发布
 ```
