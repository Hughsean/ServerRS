...
- [ ] `PARTIAL FUP-008` Agenda 写操作已支持 Owner 确认/拒绝、稍后提醒、完成、取消和改期；
  ...
  FollowUp ID 与来源版本忽略单条通用跟进，同时压制尚未领取的通知；通用跟进推迟和批量处理
  仍待实现。Owner 也可忽略线程开放问题并关闭/重开线程。
...
- [ ] `PARTIAL CMD-004` 已接入事件检索、来源回读、线程检索、指代解析、近期事项、提醒草稿、
  ...
  忽略开放问题、关闭/重开线程，以及按稳定 ID/版本忽略单条 FollowUp。L2 写操作经既有
...
- [ ] `PARTIAL OPS-003` 已通过类型化 Owner Action 展示记忆及来源、待处理承诺/项目阻塞/回复
  ...
  运行期健康详情仍待完成。
- [ ] `PARTIAL OPS-004` 已提供记忆修正/删除、通知策略、Agenda 和线程语义/生命周期入口，写操作
  以及单条 FollowUp 忽略入口，写操作均有不可变审计与 Effect Receipt；失败派生任务的有界
  重新处理入口仍待实现。
