...
> 当前开发阶段：在 `deepseek/qqbot-owner-work-close-v1` 连续收口全部
...
- [x] `DONE FUP-002` 已确认承诺与等待回复均生成幂等候选；等待回复会以“是否需要处理”的
  Owner 提醒进入策略链。Owner 可按明确 ID/来源版本提前完成单条或最多 20 条 FollowUp，也可
  单条或批量关闭不再需要处理的回复期待；全部操作均压制尚未投递通知且不会被后续扫描复活。
...
- [x] `DONE FUP-008` Agenda 写操作已支持 Owner 确认/拒绝、稍后提醒、完成、取消和改期；
  ...
  现亦可提前完成单条或最多 20 条 FollowUp，并单条或批量关闭回复期待；完成/关闭没有自动撤销
  入口，因此策略元数据不会将其标记为可逆。Owner 也可忽略线程开放问题并关闭/重开线程。
