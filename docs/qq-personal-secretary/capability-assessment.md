...
> 审计日期：2026-07-23
> 审计基线：`codex/qq-personal-secretary` 未提交工作区
> 结论：目标可实现；当前已具备消息事实存储和连续性审计底座，但历史回补、因果线程、
> 结构化记忆和主动跟进闭环尚未形成。
...
| 判断消息回复了什么 | 部分具备 | `Reply` 的账号视角 ID 已实测，并解析已入库父消息的 `reply_to_event_id` | 父消息后到时仍需回填 |
| 讨论前因后果 | 不具备 | 已有 `SourceEvent`，但没有 `EventThread`、Claim 或 Decision | 当前仍是逐条事件处理 |
| 谁提出要求 | 不具备 | 有发送者身份，但没有要求/承诺提取模型 | 可实现，必须保存来源事件 |
| 结论、分歧和话题结束 | 不具备 | 没有线程状态和结论修订链 | 可实现，不能仅依赖一次 LLM 摘要 |
| 同一事项跨群、跨私聊关联 | 不具备 | 会话 ID 已隔离，但没有实体/主题链接 | 可实现，需候选链接、置信度和 Owner 确认 |
| 服务器空窗恢复 | 部分具备 | 已持久化 ConnectionEpoch、账号/会话 Cursor 和 uncertain Gap | 无历史回补端口，仍不能保证不丢消息 |
| 主动检查事项是否完成 | 不具备 | 没有 Commitment、FollowUpRule、Scheduler 或 Reminder | 可实现，应由持久化 Worker 驱动 |
| 人物/项目/承诺结构化记忆 | 不具备 | 已有可追溯消息事实，但没有秘书业务记忆模型 | 可实现，不能复用数字人的自由文本记忆代替 |
| 记忆来源、修改、删除、TTL | 不具备 | 无秘书记忆仓储和策略 | 必须作为数据模型的一等能力 |
| 自然语言控制和反馈 | 权限底座已具备 | 已区分 `OwnerCommand` 与 NapCat Observation | 尚无 QQ 开放平台适配器、Action 和业务服务 |
| Agent Graph、Effect、暂停恢复 | 通用底座具备 | `agent-core` 有类型化状态、Graph、Effect、Checkpoint、Suspend/Resume | 可复用，但没有个人秘书业务状态和节点 |
| 跨进程 Checkpoint | 数字人已有实现 | 数字人业务有 MySQL Checkpoint Store | 不能直接共享数字人业务模型；可复用实现模式 |

因此，当前系统仍不能把跨群聊消息建模为完整因果链，也不能主动跟进。下一步应实现历史回补
Worker 和 Gap 完整性判定，再开发因果和记忆层。
