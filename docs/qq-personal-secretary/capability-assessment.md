...
> 跟进 Outbox、QQ 开放平台协议适配和类型化 Agent 动作安全底座。自然语言规划与真实联机尚未完成。
...
| Agent Graph、Effect、暂停恢复 | 业务底座具备 | 个人秘书已有有界状态和 L0-L3 策略；L2/L3 通过 NodeResult::Suspend | 通用执行节点与 Prompt 装配待接入 |
