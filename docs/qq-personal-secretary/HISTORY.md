...
- **2026-08-09 20:41（发布配置分批归档）**：将 Compose、Docker 停机宽限、增量迁移入口、
  DeepSeek/Backfill/主动通知安全默认值、本机管理员端口和公开配置说明独立成发布配置提交；示例只含
  占位值，未提交本机 `.env`、数据库密码、平台凭据或 `.mcp.json`。
...
- **本轮（DeepSeek 官方 Provider）**：`[llm]` 新增显式 `deepseek` Provider，端点固定为官方
  ...
  Clippy 通过；专用密钥设置后，官方 API 的来源约束语义提取与提示注入边界真实测试 2/2 通过。
- **本轮（CMD-002/CMD-LIVE 完成）**：本地忽略 `.env` 的三项 QQ 开放平台凭据已更新；
  ...
  用户随后向机器人发起 C2C 消息，基于 Gateway 权威消息 ID 的被动回复成功并取得非空平台回执。
  客户端现已提供受约束的被动文本回复边界，C2C/群聊序号、消息 ID 和正文均在请求前验证。
- **本轮（ENV-003，已完成）**：用户把 6099 对应账号的唯一授权群设置为
  `4（接收不提醒）`；权威状态读取确认后先建立 6099 WebSocket，再由 6100 向该群发送唯一标记，
  发送成功且 6099 收到同一消息。结合此前自身消息上报、历史回读及隔离 MySQL 完整派生链证据，
  ENV-003 三项实机验收全部闭合；测试消息按授权保留。
- **本轮（本地上线门禁 + ENV-003 边界复核）**：独立重跑格式、空白、workspace 全目标编译、
  QQBot 五个 crate 严格 Clippy、领域 293/293、NapCat 71/71、QQBot Server 198 passed/3 ignored 和
  workspace boundaries 24/24；Docker MySQL 保留的 19 个目标共 52/52 场景真实通过，随机 schema
  全部清理。6099/6100 的授权群 `cmdUinMsgMask` 均为 `1`，NapCat 4.18.14 未注册设置群消息接收方式
  的 OneBot action，因此当时等待用户在 QQ 客户端执行免打扰切换；该验收随后于 22:17 完成。未来
  远端发布治理 `QA-004` 按同一产品范围决定取消，不再保留为当前 TODO。
- **本轮（双账号恢复复核 + 产品范围收口）**：6099/6100 重新登录后只读授权群历史；6099 连续页
  `10,10,10,10,10,5,1`，6100 为 `10,10,10,10,10,6,1`，两边均因包含式锚点不推进而
  `no_progress`。交叉 cursor 返回目标账号自己的 1 条非同锚点结果，继续禁止跨账号复用。两实例
  PacketBackend 都明确报告当前 QQ 版本不兼容。知识库、多模态理解和第三方自动回复按产品决定停止，
  未进入代码或迁移。
...
- **本轮（CMD-004，已完成）**：在 Action Planner 适配器增加高置信度中文只读意图的严格完整
  ...
  子串或关键词猜测。混合写命令、目标/版本/时间不完整命令、SQL/发送意图和 Replan 继续进入
  受约束 Planner。表驱动正向与负向测试 2/2 通过。
