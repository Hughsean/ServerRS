...
- **本轮（ENV-002 完成，CMD-002 部分完成）**：本地忽略 `.env` 的三项 QQ 开放平台凭据已更新；
  不输出值的结构检查通过，真实 Token、Gateway Identify 和保留同一 Session 的 Resume 均成功。
  Owner C2C 发送返回官方 `500/11255`，现有分类正确收敛为 `unknown_commit` 且未盲重试；真实投递与
  交互回执仍需用户对账或先向机器人发消息，CMD-002/CMD-LIVE 保持未完成。
