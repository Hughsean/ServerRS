...
Glue modules:
  - bootstrap/ : dependency injection and runtime assembly
  - shared/    : config, error types, shared utilities
```
...
│   └── refresh_token_revocation_repo.rs 令牌吊销接口
│
├── conversation/
│   ├── mod.rs
│   ├── conversation.rs        会话数据结构
│   ├── conversation_message.rs 消息数据结构
│   └── conversation_repo.rs ★ 会话仓库接口
│
├── memory/
│   └── mod.rs                  记忆数据结构
│
...
│       ├── user_context_version.rs / user_context_control.rs
│       ├── conversation.rs / conversation_summary.rs
│       ├── memory.rs / rag.rs / vector_index.rs
│       ├── risk.rs / agent.rs / refresh_token_store.rs
│       ├── music.rs / diary.rs / depression.rs
│       ├── psychology.rs / community.rs / like.rs
│       ├── stored_object.rs
│       └── stub.rs              测试用桩实现
│
├── llm/                           AI 模型实现
...
| Body: {"message":"How is the weather today?"}                    |
+------------------------------------------------------------------+
        |
        v
+------------------------------------------------------------------+
| 2. src/api/router.rs                                             |
| Match route and call chat_send_message.                          |
+------------------------------------------------------------------+
        |
        v
+------------------------------------------------------------------+
| 3. src/api/middleware/auth_middleware.rs                         |
| Parse JWT token, verify identity, then continue.                 |
+------------------------------------------------------------------+
        |
        v
+------------------------------------------------------------------+
...
| 2) Call chat_service.send_message()                              |
+------------------------------------------------------------------+
        |
        v
+------------------------------------------------------------------+
...
| 1) Lock by user_id and find/create the only session              |
| 2) Load recent message history from database                     |
| 3) Call AgentRuntime to generate a reply                         |
| 4) Publish TurnClosedEvent and return result                     |
+------------------------------------------------------------------+
        |
        v
+------------------------------------------------------------------+
...
| 1) Append current user message and trim context                  |
| 2) Build summary, memory, RAG, and persona context               |
| 3) Build system prompt and tool descriptions                     |
| 4) Call LLM via Ollama/OpenAI-compatible API                     |
| 5) LLM decides whether a tool is needed                          |
| 6) Call get_weather_tool when weather is detected                |
| 7) Send tool result back to LLM for final answer                 |
| 8) Atomically save user message and AI reply                     |
| 9) Extract long-term memory asynchronously                       |
+------------------------------------------------------------------+
        |
        v
+------------------------------------------------------------------+
...
| GetWeatherTool::call():                                          |
| 1) Query QWeather API for Hefei real-time weather                |
| 2) Return normalized weather data                                |
+------------------------------------------------------------------+
        |
        v
+------------------------------------------------------------------+
| 8. LLM final reply                                               |
| "Hefei is 28 C and sunny. A short walk looks good."              |
+------------------------------------------------------------------+
        |
        v
+------------------------------------------------------------------+
| 9. Background tasks                                              |
| 1) SummaryRefreshHandler checks summary refresh                  |
| 2) PostConversationRiskAuditWorker checks risk signals           |
| 3) MemoryExtractor extracts long-term memory                     |
+------------------------------------------------------------------+
        |
        v
+------------------------------------------------------------------+
| 10. HTTP response                                                |
| { "reply": "Hefei is 28 C and sunny...",                         |
|   "tool_calls": [...] }                                          |
+------------------------------------------------------------------+
```
...
```
main.rs
  `-- bootstrap::runtime::run()        (6 startup stages)
      |
      |-- 1. bootstrap::infra          SSH tunnels, DB, LLM provider
      |-- 2. bootstrap::repos          infra/repo + domain traits
      |-- 3. bootstrap::tasks          task publisher, workers, limits
      |-- 4. bootstrap::vector         embeddings, Qdrant, vector index
      |-- 5. bootstrap::state          Chat/Memory/RAG/Agent services
      `-- 6. API / HTTP Serve          Axum routes, middleware, shutdown
          |
          |-- api/                     HTTP layer
          |   |-- router.rs            registers routes from handlers/*
          |   |-- handlers/*           calls app services
          |   `-- middleware/          checks JWT identity
          |
          |-- app/                     business logic layer
          |   |-- */*_service.rs       calls domain traits
          |   `-- agent/               uses memory, rag, summary, tools
          |
          |-- domain/                  pure models + traits
          |   `-- */                   shared by app and infra
          |
          `-- infra/                   implementations of domain traits
