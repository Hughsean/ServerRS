...
impl ProactiveEvaluator {
    ...
    async fn tick(&self) -> Result<(), QqBotError> {
        let groups = self.group_repo.find_enabled_by_bot(self.bot_account_id)
            .await
    ...
    async fn evaluate_group(&self, group: &GroupConfig, now: i64) -> Result<(), QqBotError> {
        ...
        // 获取最近消息
        let recent = self.message_repo.recent_by_group(group_id, 5).await
    ...
    fn layer1_filter(
        ...
        // 只检查群友发的消息（忽略自己发的）
        let user_msgs: Vec<&NormalizedMessage> = recent.iter()
        ...
        let has_image = last_user_msg.segments.iter().any(|s| {
            matches!(s, crate::domain::qq_bot::message::MessageSegment::Image { .. })
        });
        ...
        if is_question && user_msgs.len() < 2 {
            return Some(ProactiveIntent {
                group_id: _group_id,
                action_type: ProactiveAction::FillSilence,
                reasoning: format!("有人提问「{}」但无人回应", &last_user_msg.normalized_text.chars().take(20).collect::<String>()),
            });
        }
        ...
        if last_user_msg.at_bot {
            return Some(ProactiveIntent {
                group_id: _group_id,
                action_type: ProactiveAction::FillSilence,
                reasoning: format!("有人@了我，消息是「{}」", &last_user_msg.normalized_text.chars().take(30).collect::<String>()),
            });
        }

        // 规则 5：话题中断 — 最后 3 条消息来自同一个人连续说，突然停了
        let same_user_consecutive = user_msgs.windows(3).any(|w| {
            w.iter().all(|m| m.qq_user_id == w[0].qq_user_id)
        });
    ...
    async fn layer2_judge(
        ...
        let topic = self.topic_service.get_current_topic_label(group_id).await
...
请输出 yes 或 no。"#,
    ...
    async fn execute_proactive(&self, group_id: i64, _intent: &ProactiveIntent) -> Result<(), QqBotError> {
        // 检查注意力状态
        ...
        // 生成主动回复
        let reply = self.reply_generator.generate_proactive_reply(context).await?;

        // 分发
        match self.segment_dispatcher.send_direct(group_id, &reply, None).await {
            Ok(_) => {
                info!(
                    group_id,
            ...
            Err(e) => {
                ...
                let _ = self.segment_dispatcher.enqueue_reply(group_id, &reply, None).await;
            }
        }
