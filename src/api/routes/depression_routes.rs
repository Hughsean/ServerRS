...
pub fn depression_routes(state: ApiState) -> Router {
    Router::new()
        ...
        .route("/api/v1/depression/assessments", get(list_assessments).post(create_assessment))
        .route("/api/v1/depression/assessments/{id}", get(get_assessment).delete(delete_assessment))
        .with_state(state)
}
