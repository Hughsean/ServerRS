...
```text
database/
├── baseline/20260803_qqbot_schema_v1.sql  # 全新数据库的最终结构
├── migrations/                            # Baseline v1 之后的增量迁移
├── archive/pre_v1/                        # 压缩前 33 个历史迁移，仅供审计/旧库核对
└── test_support/qqbot_migrations.rs        # 隔离 MySQL 测试共用加载器
```

Baseline v1 包含 78 张 `secretary_*` 表和 2 个 View，只保存最终 DDL，不含业务数据、测试数据、
凭据或历史 `ALTER/DROP` 过程。

## 执行规则

- 全新空库：先执行 `baseline/20260803_qqbot_schema_v1.sql`，再按文件名字典序执行
  `migrations/` 中的增量文件。
- 已完整执行压缩前 33 个迁移的数据库：不得重放 Baseline。确认旧迁移记录完整后，将其视为
  已采用 Baseline，再执行后续增量迁移。
- 只有部分旧迁移、已有 `secretary_*` 表却没有完整迁移记录的数据库：必须 fail-closed，先人工
  核对结构；不得把 Baseline 覆盖到未知或部分结构上。
- `archive/pre_v1/` 永不参与全新数据库加载，也不得为了修复单表而从中选择性重放。
- 后续每个结构变化继续新增一个小型增量 SQL；不要持续改写 Baseline v1。
