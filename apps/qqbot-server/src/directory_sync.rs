//! B4 账号会话目录同步 Worker。
//!
//! 周期性后台任务：调用 NapCat 只读列表 API（`get_friend_list`/`get_group_list`/
//! `get_recent_contact`），聚合为 `DirectorySnapshot`，持久化到 MySQL。
//!
//! 关键约束：
...
use personal_secretary::{
    DirectoryListEntry, DirectorySourceError, DirectorySourceT, DirectorySyncError,
    DirectorySyncUseCase, ScopeBoundary, ScopeKind, SourceAccountRef,
};
...
/// NapCat `DirectorySourceT` 适配器：把只读列表 API 返回转换为协议无关 `DirectoryListEntry`。
///
/// 所有平台 ID 转为字符串保留精度；`peerUin`/`msgTime` 已在 DTO 层以字符串接收。
/// 1 MiB 上限拒绝映射为 `DirectorySourceError::Oversized`（由 `NapCatApiClient` 的
/// `MAX_RESPONSE_BYTES` 流式限流触发）。
...
/// 把 NapCat 错误映射为 `DirectorySourceError`。
fn map_napcat_error(e: NapCatError) -> DirectorySourceError {
    let msg = e.to_string();
    match &e {
        NapCatError::Protocol(s) if s.contains("oversized") || s.contains("exceeds") => {
            DirectorySourceError::Oversized(msg)
        }
        NapCatError::Connection(s) if s.contains("timeout") || s.contains("Timeout") => {
            DirectorySourceError::Timeout(msg)
        }
        NapCatError::Connection(_) => DirectorySourceError::Transient(msg),
        NapCatError::Protocol(s) if s.contains("retcode") || s.contains("unavailable") => {
            DirectorySourceError::Unavailable(msg)
        }
        NapCatError::Protocol(_) => DirectorySourceError::Malformed(msg),
        _ => DirectorySourceError::Transient(msg),
    }
}
...
impl DirectorySourceT for NapCatDirectorySource {
    async fn list_friends(
        ...
        Ok(friends
            .into_iter()
            .map(|f| DirectoryListEntry {
                platform_id: f.user_id.to_string(),
                display_name: if f.remark.is_empty() {
    ...
    async fn list_groups(
        ...
        Ok(groups
            .into_iter()
            .map(|g| DirectoryListEntry {
                platform_id: g.group_id.to_string(),
                display_name: if g.group_name.is_empty() {
                    None
                } else {
                    Some(g.group_name)
                },
    ...
    async fn list_recent_contacts(
        ...
        Ok(contacts
            .into_iter()
            .map(|c| DirectoryListEntry {
                // peer_uin 已是字符串，保留精度。
                platform_id: c.peer_uin.clone(),
                display_name: if c.peer_name.is_empty() {
                    None
                } else {
                    Some(c.peer_name.clone())
                },
                // msg_time 已是字符串，作为边界。
                boundary: if c.msg_time.is_empty() {
                    None
                } else {
                    Some(ScopeBoundary::new(String::new(), c.msg_time.clone()))
                },
                // chatType: 1=私聊, 2=群聊。
                kind_hint: if c.chat_type == 2 {
                    ScopeKind::Group
                } else {
                    ScopeKind::Friend
                },
...
mod tests {
    use super::*;
    use personal_secretary::{
        DirectorySnapshot, DirectoryStoreError, DirectoryStoreT, IngestionGapId, MessageSource,
    };
