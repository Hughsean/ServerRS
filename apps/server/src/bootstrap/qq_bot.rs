...
use crate::domain::qq_bot::{
    AttentionStore, ExternalIdentity, GroupMessageGateway, GroupMessageHandler,
    PlatformUserDirectory, PlatformUserId, QqBotError,
};
...
struct DigitalHumanUserDirectory {
    users: Arc<dyn UserRepoT>,
}

#[async_trait::async_trait]
impl PlatformUserDirectory for DigitalHumanUserDirectory {
    async fn resolve_or_create(
        &self,
        identity: ExternalIdentity,
    ) -> Result<PlatformUserId, QqBotError> {
        let username = format!("qq_{}", identity.external_user_id);
        let user = NewUser::new_without_password(&username, identity.display_name);
        self.users
            .save(user)
            .await
            .map(|user| PlatformUserId(user.id))
            .map_err(|error| {
                QqBotError::Internal(format!(
                    "failed to resolve platform user {}: {error}",
...
pub async fn init_qq_bot(
    ...
    let pb_user_directory = user_repo.map(|users| {
        Arc::new(DigitalHumanUserDirectory { users }) as Arc<dyn PlatformUserDirectory>
    });
    ...
    let profile_builder = if qc.profile_enabled {
        if let (Some(user_directory), Some(external_user_repo), Some(user_profile_repo)) = (
            pb_user_directory,
            pb_external_user_repo,
            pb_user_profile_repo,
        ) {
