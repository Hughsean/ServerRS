...
use server_rs::database::init_db;
...
async fn main() {
    ...
    let repos = qqbot::repositories::build_repositories(&db);
    let bot_account_repo = &repos.bot_account;
    let group_repo = &repos.group;
    let group_member_repo = &repos.group_member;
    let external_user_repo = &repos.external_user;
