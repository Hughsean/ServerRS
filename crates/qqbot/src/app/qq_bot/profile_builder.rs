...
use crate::domain::qq_bot::{ExternalIdentity, PlatformUserDirectory, PlatformUserId};
...
pub struct ProfileBuilder {
    user_directory: Arc<dyn PlatformUserDirectory>,
...
impl ProfileBuilder {
    ...
    pub async fn ensure_user_registered(
            ...
            .map_err(|e| QqBotError::Internal(format!("failed to find external user: {e}")))?
        {
            ...
            if existing.internal_user_id.is_none() {
                let platform_user_id = self.create_platform_user(qq_user_id, nickname).await?;
                let updated = ExternalUser {
                    internal_user_id: Some(platform_user_id.0),
                    ..existing
                };
        ...
        info!(
            qq_user_id,
            platform_user_id = platform_user_id.0,
    ...
    async fn create_platform_user(
        ...
        self.user_directory
            .resolve_or_create(ExternalIdentity {
                external_user_id: qq_user_id.to_string(),
                display_name: nickname.map(str::to_owned),
            })
            .await
    }
}
