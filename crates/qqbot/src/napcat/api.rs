...
impl NapCatApiClient {
    ...
    pub async fn group_poke(&self, group_id: i64, user_id: i64) -> Result<(), NapCatError> {
    ...
    pub async fn send_group_msg(
        ...
        group_id: i64,
        message: &str,
    ) -> Result<SendGroupMsgData, NapCatError> {
        let params = serde_json::json!({
            "group_id": group_id,
            "message": message,
            "auto_escape": false,
        ...
        Ok(serde_json::from_value(resp.data.unwrap_or_default())
            .map_err(|e| NapCatError::Protocol(format!("parse send_group_msg data: {e}")))?)
    }
