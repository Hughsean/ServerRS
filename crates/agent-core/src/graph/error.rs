...
impl NodeError {
    ...
    /// Preserve a typed caller error without coupling the runtime to a business crate.
    pub fn with_source<E>(kind: NodeErrorKind, error: E) -> Self
    where
...
mod tests {
    ...
    #[test]
    fn typed_source_is_preserved_without_business_dependency() {
        let error = NodeError::with_source(
