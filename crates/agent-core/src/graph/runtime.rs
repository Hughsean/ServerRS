...
use crate::{AgentBusinessState, AgentState};
use std::sync::Arc;
...
mod tests {
    use super::super::*;
    use crate::{AgentBusinessState, AgentOutcome, AgentState, AgentStateError, AgentUpdate};
    ...
    #[error("conflict: {0}")]
    struct TestApplicationError(String);
    ...
    impl EffectExecutor<TestEffect> for TestEffectExecutor {
        async fn execute(
            ...
            match &envelope.effect {
                ...
                TestEffect::Fail => Err(EffectError::with_source(
                    EffectErrorKind::Permanent,
                    TestApplicationError("turn changed".into()),
                )),
    ...
    enum Behavior {
        ...
        Pending,
        ApplicationFailure(TestApplicationError),
    }
    ...
    impl AgentNode<TestBusiness> for FakeNode {
        ...
        async fn execute(
            ...
            match self.behavior {
                ...
                Behavior::ApplicationFailure(ref error) => Err(NodeError::with_source(
    ...
    #[tokio::test]
    async fn failed_effect_is_not_retried_and_preserves_typed_source() {
        let executor = Arc::new(TestEffectExecutor::recording());
        let runtime = GraphRuntime::with_effect_executor(
        ...
        match error {
            GraphRunError::EffectFailed { error, .. } => assert!(matches!(
                error.source_ref::<TestApplicationError>(),
                Some(TestApplicationError(message)) if message == "turn changed"
            )),
            other => panic!("unexpected error: {other:?}"),
        }
    }
    ...
    #[tokio::test]
    async fn preserves_typed_source_through_runtime() {
