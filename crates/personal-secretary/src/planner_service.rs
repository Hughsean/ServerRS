...
use agent_core::graph::{GraphExecutionResult, RunBudget};
use thiserror::Error;
...
/// P0-3 修复：持有 DatabaseConnection，per-run 构造绑定业务 ActionRunId 的 CheckpointStore。
...
impl PlannerUseCase {
    pub fn new(
        store: Arc<dyn ActionStoreT>,
        planner: Arc<dyn crate::ActionPlannerT>,
        _checkpoint_store: Arc<dyn agent_core::graph::CheckpointStore<SecretaryAgentState>>,
        lease_secs: u64,
    ) -> Self {
        Self {
            store,
    ...
    pub fn with_checkpoint_db(mut self, db: sea_orm::DatabaseConnection) -> Self {
        self.checkpoint_db = Some(db);
        self
    }
    ...
    async fn execute_claimed(
        ...
        let checkpoint_store: Arc<dyn agent_core::graph::CheckpointStore<SecretaryAgentState>> =
            if let Some(db) = &self.checkpoint_db {
                crate::build_bound_action_checkpoint_store(db.clone(), run_id.clone())
            } else {
