...
//! MySQL 或 `qqbot-server`。外层（`apps/qqbot-server`）实现 [`DirectorySourceT`]，
//! 基础设施层（`crates/personal-secretary/src/infra`）实现 [`DirectoryStoreT`]。
//!
//! 用例职责：
...
impl From<sea_orm::DbErr> for DirectoryStoreError {
...
impl DirectorySyncUseCase {
    ...
    /// （`apps/qqbot-server` 的 Worker）通过 `tokio::time::timeout` 包装控制，
